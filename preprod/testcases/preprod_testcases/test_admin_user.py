...
class TestAddUser:
    def test_add_user(self,preprod_setup,preprod_test_data):
        self.driver = preprod_setup
        self.AU = AddUser(self.driver)
        self.AU.click_on_user()
        self.AU.click_on_add_user()
        self.AU.select_account(preprod_test_data['select_Account'])
        self.AU.enter_first_name(preprod_test_data['user_First_Name'])
        self.AU.enter_last_name(preprod_test_data['user_Last_Name'])
        #addiging random email
        base_email = preprod_test_data['user_Email']
        ...
        self.AU.enter_phone_number(preprod_test_data['user_Phone'])
        self.AU.enter_address(preprod_test_data['user_Address'])
        self.AU.enter_job(preprod_test_data['user_Job'])
        self.AU.select_profile_role(preprod_test_data['user_Profile'])
        self.AU.click_save_button()
        self.AU.wait_for_progress_to_disappear()
        assert self.AU.get_toast_message() == 'New user created!'

    def test_search_user(self, preprod_setup, preprod_test_data):
        self.driver = preprod_setup
        # time.sleep(10)
        ...
        for role in roles:
            ...
            self.SU.wait_for_progress_to_disappear()
            self.SU.scroll(-00)
        self.SU.enter_account_name(preprod_test_data['select_Account'])
        self.SU.perform_search_and_reset()
        self.SU.enter_first_name(preprod_test_data['us_First_Name'])
        self.SU.perform_search_and_reset()
        self.SU.enter_last_name(preprod_test_data['us_Last_Name'])
        self.SU.perform_search_and_reset()
        self.SU.terminate_search_opeartion()

    def test_delete_user(self, preprod_setup, preprod_test_data):
        self.driver = preprod_setup
        # time.sleep(10)
        self.DU = DeleteUser(self.driver)
        self.DU.click_on_user()
        self.DU.click_on_search_button()
        self.DU.enter_first_name(preprod_test_data['user_First_Name'])
